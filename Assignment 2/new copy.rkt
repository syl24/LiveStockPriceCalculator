...
; write more tests for parse as needed
...
; write more tests for checking that parse generates errors when needed
...
; The following test is just an example. Write more tests for subst!
...
(define (interp expr)
  (type-case DE expr
    ...
    ;; DELETE ME!
    ;[compop (op lexp rexp) (map (for-each (lambda (x) ((if (boolean? x) (first x) (rest x)) x))
    ;                                        ((lambda (x) (apply op x))
    ;                                        (cartesian-product (interp lexp) (interp rexp))))
    ;                                        )] ; test - does not work, helper function needed - creates a list of bools,
    ;                                             cannot extract vals 
