...
(define (parse sexp)
  (match sexp
    [(? legal-id?) (id sexp)]
    [(list 'distribution (? number? v) ...) (distribution v)]
    [(list 'distribution v empty) (distribution '())]
    [(? number?) (distribution (list sexp))]
    [(? empty?) (distribution '())]
    [(list 'uniform sexp1 sexp2) (if (<= sexp1 sexp2)
...
(test (parse 0) (distribution '(0)))

(test (parse '{distribution 1 2 3}) (distribution '(1 2 3)))
...
(test (parse '{observe-that {- 1 2} is < {+ 3 4}})
      (compop <
             (binop - (distribution '(1)) (distribution '(2)))
             (binop + (distribution '(3)) (distribution '(4)))))
(test (parse '{observe-that {+ 2 2} is == {+ 3 1}})
      (compop =
             (binop + (distribution '(2)) (distribution '(2)))
             (binop + (distribution '(3)) (distribution '(1)))))
(test (parse '{uniform 6 6}) (distribution '()))
...
;; substitute out sub-id for val everywhere in expr
(define (subst expr sub-id val)
  (type-case DE expr
    [distribution (values) expr]
    ;[uniform (expr1 expr2) (distribution (range (subst expr1 sub-id val)
    ;                                            (subst expr2 sub-id val)))]
    [binop (op expr1 expr2) (binop op (subst expr1 sub-id val)
                                (subst expr2 sub-id val))]
    [compop (comp expr1 expr2) (compop comp (subst expr1 sub-id val)
                                     (subst expr2 sub-id val))]
...
(test (subst (distribution '(1 2)) (id 'x) 3) (distribution '(1 2)))
(test (subst (binop - (distribution '(1 2)) (id 'x)) 'x (distribution '(3)))
      (binop - (distribution '(1 2)) (distribution '(3))))
(test (subst (binop * (distribution '(1 2)) (id 'x)) 'x (distribution '(3)))
      (binop * (distribution '(1 2)) (distribution '(3))))
(test (subst (compop < (id 'x) (id 'x)) 'x (distribution '(3)))
      (compop < (distribution '(3)) (distribution '(3))))
(test (subst (subst (compop < (id 'x) (id 'y)) 'x (distribution '(3))) 'y (distribution '(5)))
      (compop < (distribution '(3)) (distribution '(5))))

(test (subst (with 'x (distribution '(2)) (distribution '(3))) 'x (distribution '(1)))
      (with 'x (distribution '(2)) (distribution '(3))))
(test (subst (with 'x (id 'y) (id 'y)) 'y (distribution '(1)))
      (with 'x (distribution '(1)) (distribution '(1))))
(test (subst (with 'x (id 'x) (id 'x)) 'x (distribution '(5)))
      (with 'x (distribution '(5)) (id 'x)))
...
(define (interp expr)
  (type-case DE expr
    ...
    [binop (op expr1 expr2) (interp-binop-helper op (interp expr1) (interp expr2) empty)]
    [compop (comp expr1 expr2) (interp-compop-helper comp (interp expr1) (interp expr2) empty)]
    [with (id named-e body)
          (interp (subst body id named-e))]
    [id (name) (error 'interp "")]))

(define (interp-binop-helper op expr1 expr2 list)
  (if (empty? expr1)
      list
      (interp-binop-helper op (rest expr1) expr2
                           (append list (interp-binop-helper-helper op (first expr1) expr2 '())))))

(define (interp-binop-helper-helper op val1 expr2 list)
  (if (empty? expr2)
      list
      (interp-binop-helper-helper op val1 (rest expr2) (cons (op val1 (first expr2))  list))))

(define (interp-compop-helper comp expr1 expr2 list)
  (if (empty? expr1)
      list
      (interp-compop-helper comp (rest expr1) expr2
                            (append list (interp-compop-helper-helper comp (first expr1) expr2 '())))))

(define (interp-compop-helper-helper comp val1 expr2 list)
  (if (empty? expr2)
      list
      (interp-compop-helper-helper comp val1 (rest expr2)
                                   (if (comp val1 (first expr2))
                                       (cons val1 list)
                                       list))))
